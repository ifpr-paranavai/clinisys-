...
public class PermissaoControle {
	...
	public ModelAndView salvar(@Valid Permissao permissao, BindingResult result, HttpSession session) {
		try {
			...
			if (permissao1.isEmpty()) {
				...
				session.setAttribute("mensagem", new Mensagem("PERMISSÃO cadastrada com SUCESSO!!!", "success"));
			} else {
